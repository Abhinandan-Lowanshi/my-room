...
public class RouteDrawerTask extends AsyncTask<String, Integer, List<List<HashMap<String, String>>>> {
    ...
    private int routeColor,status;

    public RouteDrawerTask(GoogleMap mMap,int status) {
        this.mMap = mMap;
        this.status = status;
    }
    ...
    private void drawPolyLine(List<List<HashMap<String, String>>> result) {
        ...
        for (int i = 0; i < result.size(); i++) {
            ...
            lineOptions.width(6);
            if(status==1) {
                routeColor = ContextCompat.getColor(DrawRouteMaps.getContext(), R.color.quantum_grey);
            }
            else {
                routeColor = ContextCompat.getColor(DrawRouteMaps.getContext(), R.color.quantum_googgreen);
            }
            if (routeColor == 0)
